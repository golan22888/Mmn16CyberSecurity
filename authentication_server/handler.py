...
class RegisterHandler(Handler):
    ...
    def handle(request, client_manager):
        try:
            ...
            client_manager.update_last_seen_by_name(client_name)
            print(f'client {client_name} is registered')
            return ResponseProvider.make_response(request, 1600, id=client_manager.client_id)
...
class SymKeyRequestHandler(Handler):
    ...
    def handle(request, client_manager):
        try:
            ...
            encrypted_key = EncryptedKey(nonce, aes_key, client_manager.get_client_by_id())
